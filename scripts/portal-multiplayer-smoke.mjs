...
for (const caseStudy of selectedCases) {
  ...
  try {
    ...
    results.push({ case: caseStudy.country, hazard: caseStudy.hazardLabel, channelId, users: users.length, messages: expectedMessages, presence: handles[0].presence?.count ?? 0 });
  } finally {
...
console.log(`Portal multijugador verificado: ${results.length} casos reales, ${users.length} usuarios por caso.`);
