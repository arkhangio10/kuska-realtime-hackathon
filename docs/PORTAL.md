...
Los mensajes persistentes implementados son `proposal.created`, `alternative.created`, `vote.cast` y `chat.created`. `alternative.created` comparte con toda la sala las propuestas puente y opciones promovidas desde el análisis, manteniendo separada la identidad humana que la publicó de la autoría de facilitación. Cada evento tiene un `eventId` para deduplicación, se valida con Zod al entrar y debe ocupar menos de 1,900 bytes para mantenerse por debajo del límite de 2 KB del SDK. El historial recupera hasta 80 eventos para quienes llegan tarde. Los mensajes propios se aplican de forma optimista y su eco se descarta por `eventId`.
