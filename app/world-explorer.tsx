...
const motionLabels:Record<CaseStudy["hazardKind"],string>={flood:"CORRIENTE Y NIVEL VARIABLES",earthquake:"PULSOS SÍSMICOS Y RÉPLICAS",cyclone:"RÁFAGAS Y DEBRIS EN ROTACIÓN",volcano:"LAVA, EYECCIONES Y CENIZA",wildfire:"BRASAS, HUMO Y PROPAGACIÓN",drought:"POLVO Y ESTRÉS HÍDRICO",chemical:"FUGA Y NUBE CONTAMINANTE",biological:"PERÍMETRO SANITARIO ACTIVO",radiological:"PULSO DE ZONA RADIOLÓGICA",transport:"FUEGO, CHISPAS Y HUMO",other:"RIESGO AMBIENTAL ACTIVO"};
...
export function WorldExplorer({ alias, role, onBack, onOpenMission, onClearScenario, caseStudy, scenario }: WorldExplorerProps) {
  ...
  useEffect(() => {
    ...
    const riverEnabled=kind==="flood"||kind==="cyclone"||kind==="drought";
    ...
    const scorched = new THREE.MeshLambertMaterial({color:kind==="volcano"?0x493f3a:0x4b432f});
    ...
    for (let x = -13; x <= 13; x++) {
      for (let z = -13; z <= 13; z++) {
        ...
        const hazardousGround=hazardDistance<(kind==="volcano"?5.4:kind==="wildfire"?4.8:3.7);
        const mat = hazardousGround&&(kind==="volcano"||kind==="wildfire")?(kind==="volcano"&&hazardDistance>3.8?ashSoil:scorched):hazardousGround&&(kind==="chemical"||kind==="radiological"||kind==="transport"||kind==="biological")?hazardConcrete:Math.abs(z - 4) < 1 && x < -1 ? road : ((x + z) % 3 ? grassA : grassB);
        const tile = block(mat, [.96, height, .96], [x, height / 2 - .62, z]);
        scene.add(tile);
        if ((x * 7 + z * 11) % 29 === 0 && !edge && !(hazardousGround&&(kind==="volcano"||kind==="wildfire"))) {
    ...
    const rainMaterial=new THREE.PointsMaterial({ color: 0xd7edeb, size: .065, transparent: true, opacity: .75 });
    ...
    const accentColor=kind==="volcano"||kind==="wildfire"||kind==="transport"?0xff8a28:kind==="drought"||kind==="earthquake"?0xd6b06c:kind==="chemical"||kind==="radiological"?0xb6df37:kind==="biological"?0xe86d61:0xd9eeee;
    ...
    const clearSky=new THREE.Color(0xa9c8bd), stormSky=new THREE.Color(0x536e6c), smokeSky=new THREE.Color(0x665f54),targetSky=new THREE.Color();
    ...
    const animate = () => {
      ...
      const accentFactor=Math.max(hazard.fire,kind==="cyclone"?hazard.wind*.8:0,kind==="drought"?hazard.drought*.65:0,kind==="earthquake"?quakePulse:0,kind==="chemical"||kind==="radiological"||kind==="biological"?hazard.contamination*.48:0);accentParticles.visible=accentFactor>.04;accentGeometry.setDrawRange(0,Math.round(accentFactor*accentCount));accentMaterial.opacity=.35+accentFactor*.58;
      targetSky.copy(clearSky).lerp(stormSky,Math.max(hazard.rain*.8,hazard.wind*.65)).lerp(smokeSky,Math.max(hazard.smoke,hazard.ash)*.72);(scene.background as THREE.Color).lerp(targetSky,.02);
      ...
      for(let i=0;i<rainCount;i++){let y=positions.getY(i)-dt*(8+hazard.wind*4),x=positions.getX(i)+dt*hazard.wind*3;if(y<0)y=18;if(x>18)x=-18;positions.setXY(i,x,y)} positions.needsUpdate=true;
  ...
  return <main className="explorer">
    ...
    {scenario?<section className="scenario-result" aria-live="polite">
      ...
      {sceneElements.length>0&&<section className="scenario-scene-map"><small>DECISIÓN → ANIMACIÓN</small><p>Los números y colores coinciden con las etiquetas que flotan sobre el territorio.</p><div className="scenario-plan-elements">{sceneElements.map((element,index)=><div className={index===activeSceneElement?"active":index<activeSceneElement?"done":""} data-scene-type={element.type} key={`${element.type}-${index}`}><i>{index+1}</i><span><b>{sceneElementIcons[element.type]??"·"} {element.label}</b><small>{sceneElementDescriptions[element.type]}</small></span></div>)}</div></section>}
    ...
    <aside className="weather-visual-key"><small>{caseStudy.dataState==="live"?"● SEÑAL EN VIVO":"SEÑAL RECIENTE"} → MUNDO</small><h3>{caseStudy.location}, {caseStudy.country}</h3><span className="visual-updated">Actualizada {formatActivityDate(caseStudy.lastActivityAt)}</span><span className="motion-status"><i/>{motionLabels[caseStudy.hazardKind]}</span>{caseStudy.metrics.slice(0,3).map(metric=><div key={metric.label}><span>{metric.label} <b>{metric.value}</b></span><i><em style={{width:`${Math.min(100,metric.level)}%`}}/></i></div>)}<p>{caseStudy.hazardKind==="volcano"?"La alerta controla lava, ceniza, humo y el perímetro de la ladera.":caseStudy.hazardKind==="earthquake"?"La magnitud controla vibración, grietas y escombros.":caseStudy.hazardKind==="wildfire"?"El área detectada controla árboles quemados, focos, humo y perímetros.":caseStudy.hazardKind==="drought"?"La exposición controla sequedad del suelo y nivel del cauce.":caseStudy.origin==="human"?"El tipo y severidad controlan instalaciones, humo, fuego o contaminación.":"El tipo y la severidad controlan agua, viento, cielo y zonas de riesgo."}</p></aside>
